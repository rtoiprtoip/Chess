...
public class GameLogicImpl implements GameLogic, Serializable {
    ...
    public boolean isThisValidMove(Coordinates from, Coordinates to) throws SpecialMoveException {
        try {
            ...
            if (getPieceAt(to) != null && getPieceAt(to).getColor() == whoseMove) {
                return false;
            }
    ...
    private void setPieceAt(Coordinates c, Piece p) {// just for setting up tests
    ...
    @Setter(onMethod = @__(@Deprecated)) // for testing only
